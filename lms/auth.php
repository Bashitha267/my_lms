...
if (isset($_POST['login'])) {
    ...
    if ($result->num_rows === 1) {
        ...
        // Uncomment to debug:
        // error_log("Login attempt - Username: $username, Hash length: " . strlen($password_hash) . ", Hash starts with: " . substr($password_hash, 0, 10));
        ...
        if (!empty($password_hash) && password_verify($password, $password_hash)) {
            ...
            // This is the key to Single Login. Every login generates a new random string.
            ...
            if ($update_stmt->execute()) {
                ...
                if (WHATSAPP_ENABLED && !empty($user['whatsapp_number']) && !empty(WHATSAPP_API_URL)) {
                    try {
                        $user_name = !empty($user['first_name']) ? $user['first_name'] : $user['username'];
